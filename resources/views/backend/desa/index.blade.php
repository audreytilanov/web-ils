                                ...
                                <tbody>
                                    @foreach ($contents as $content)
                                        <tr>
                                            ...
                                            <td>
                                                <div class="d-flex">
                                                    @if (auth()->user()->userHasPermissions->where('component_name', 'desa')->where('permissions', 'can edit')->isNotEmpty())
                                                        <a href="{{ Route('desa.edit', $content->id) }}"
                                                            class="btn btn-primary shadow btn-xs sharp mr-1"><i
                                                                class="fa fa-pencil"></i></a>
                                                    @endif
                                                    @if (auth()->user()->userHasPermissions->where('component_name', 'desa')->where('permissions', 'can delete')->isNotEmpty())
                                                        <div class="sweetalert">
                                                            <form action="{{ Route('desa.delete', $content->id) }}"
                                                                method="POST">
                                                                @csrf
                                                                <button type="submit"
