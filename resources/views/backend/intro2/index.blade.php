            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header">
                        ...
                        @if (auth()->user()->userHasPermissions->where('component_name', 'intro2')->where('permissions', 'can add')->isNotEmpty())
                            <a href="{{ Route('add_contentIntro2') }}">
                                ...
                                <tbody>
                                    @foreach ($contents as $content)
                                        <tr>
                                            ...
                                            <td><img src="{{ URL::asset('asset/intro2/' . $content->image) }}"
                                            ...
                                            <td>
                                                <div class="d-flex">
                                                    @if (auth()->user()->userHasPermissions->where('component_name', 'intro2')->where('permissions', 'can edit')->isNotEmpty())
                                                        <a href="{{ Route('edit_contentIntro2', $content->id) }}"
                                                            class="btn btn-primary shadow btn-xs sharp mr-1"><i
                                                                class="fa fa-pencil"></i></a>
                                                    @endif
                                                    @if (auth()->user()->userHasPermissions->where('component_name', 'intro2')->where('permissions', 'can delete')->isNotEmpty())
                                                        <div class="sweetalert">
                                                            <form
                                                                action="{{ Route('delete_content_intro2', $content->id) }}"
                                                                method="post">
                                                                @csrf
                                                                <button type="submit"
