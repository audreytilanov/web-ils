<footer class="site-footer">
      <div class="container">
        <div class="row">
          <div class="col-lg-4 col-md-4 col-sm-12">
            <h5 class="text-white">
              <!-- <i class="bi-geo-alt-fill me-2"></i> -->
              BaliDriveTour.com</h5>
              <h6 class="text-white col-sm-12">Jl. By Pass I Gusti Ngurah Ray, Mumbul, Nusa Dua</h6>

              
          </div>
          <br>
          <br>
          <div class="col-lg-4 col-md-4 col-sm-12">
            <h6 class="text-white col-sm-12">Whatsapp : <span style="color: orange">081237530038</span></h6>
            <h6 class="text-white col-sm-12">Contact Person : <span style="color: orange">081375042155</span></h6>
            
            {{-- <h6 class="text-white col-sm-12">BCA <span style="color: orange">8280006549</span> an. Richard E P Sinaga</h6> --}}
          </div>
          <div class="col-lg-4 col-lg-4 col-sm-12 ms-auto" style="margin-top:auto">
            <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3943.004203556583!2d115.20020521478487!3d-8.785673293689861!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x0%3A0xf95d67e8b6b0b0d5!2zOMKwNDcnMDguNCJTIDExNcKwMTInMDguNiJF!5e0!3m2!1sid!2sid!4v1668757074684!5m2!1sid!2sid" width="400" height="200" style="border:0;" allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
          </div>
        </div>
      </div>
      <div class="row mx-0 pt-5 mb-n4 footer-text mt-5 justify-content-center align-items-center" style="border-top: #112443 solid 2px ;">
        <div class="col-9 text-center">
          <div class="container">
            <p class="explanation mx-n4" style="font-size:18px;">Copyright @2022 Bali Drive Tour</p>
          </div>
        </div>
      </div>
    </footer>